...
def train_Rnn_model(data):
    ...
    for iter in range(nums_iter):
        ...
        output, hidden_prev = net(x, (h_0, c_0))
    ...
    plt.xlabel('训练次数')
    plt.ylabel('loss')
    plt.title('LSTM损失函数下降曲线')
    plt.show()

    return (h_0,c_0), net


(h_pre,c_pre), model = train_Rnn_model(y1[pre_segment:pre_segment+1000])   # 决定训练哪个部分
...
for i in range(0, 1000 - num_time_steps, num_time_steps):
    ...
    pred1, h1 = model(data_test, (h_pre,c_pre))
